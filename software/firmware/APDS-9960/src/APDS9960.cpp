...
bool APDS9960::init()
{
	Serial.println("in APDS::init()");
    uint8_t id;

    /* Initialize I2C */
 //   Wire.begin();  //CURT REMOVED BECAUSE THIS RESET TO ACCELEROMETER I2C - MUST CALL WIRE.BEGIN(x,y) EXTERNALLY
 ...
 //   delay(3);
    ...
    if( !(id == APDS9960_ID_1 || id == APDS9960_ID_2) ) {
     ...
     Serial.println("in APDS::init() after ID");
    ...
    Serial.println("in APDS::init() after ambient and proximity default sets");
    ...
    Serial.println("in APDS::init() after gesture default sets");
...
bool APDS9960::enableGestureSensor(bool interrupts)
{
 ...
 //   if( !setLEDBoost(LED_BOOST_300) ) {   CURT CHANGE
...
#if DEBUG
            Serial.println("right before data process");
#endif
...
#endif
            /*****************************************************
              CURT CHANGE clear FIFO if too high to prevent crash
            *******************************************************/
            if(fifo_level > 15) {
        /*        if( bytes_read == -1 ) {
                    return ERROR;
            ...
            //    uint8_t fifo_data_clear[128];
            //    fifo_data = fifo_data_clear;
