...
if(clocktime + speedMs < millis() && SLEEP_MODE != true){
  ...
  */
  apds.readProximity(proximity_apds9960);
   ...
   transmitSensorData(0);
   transmitSensorData(1);
...
*************** TRANSMIT SENSOR DATA OVER BLUETOOTH ****************** 
*********************************************************************/
void transmitSensorData( uint8_t packetPosition ){
    ...
    if(central){ // if a central is connected to peripheral
              ...
              int hand_gesture = 0; //temp dummy val
              ...
              if(packetPosition == 0){
                const unsigned char imuCharArray[20] = {
                  ...
                  (uint8_t)(TObj_compressed[14]),
                  (uint8_t)(TObj_compressed[15]),
                  (uint8_t)(TObj_compressed[16]),
                  (uint8_t)(TObj_compressed[17]),
                  (uint8_t)(TObj_compressed[18]),
                  (uint8_t)(0) //first half
                };
